...
#            if dostack == True:
    ...
    ### NEED TO MAKE COMPATIBLE WITH NAMES OF IMAGE IN SUBTRACTION PART
        ...
        ########## plot data
            ...
            print("co [:,0]: "+str(co[:,0]))
            print("co [:,1]: "+str(co[:,1]))
            co[:,0],co[:,1] = photutils.centroids.centroid_sources(data,co[:,0],co[:,1], centroid_func=photutils.centroids.centroid_2dg)
        ...
        ########## Zero point from seq stars
            ...
            if f in seqMags:
                ...
                while happy not in ('y','yes'):
                    ...
                    plt.subplots_adjust(left = 0.03, bottom = 0.074, right = 1, top = 1, wspace = 0.258, hspace = 0.086)
...
#                        hdu2 = fits.PrimaryHDU(tmp_resampled)
        ...
        ########### SN photometry
            ...
            # This is the aperture to count the photons from the source. It's usually some fraction of the full aperture
            apcircle = Circle((SNco[0], SNco[1]), aprad_opt, facecolor='none',
                    edgecolor='r', linewidth=3, alpha=1)
            ax4.add_patch(apcircle)

            # Aperture with radius defined by aprad
            skycircle1 = Circle((SNco[0], SNco[1]), aprad, facecolor='none',
                    edgecolor='g', linewidth=2, alpha=1)

            #Aperture with radius aprad+skyrad
            skycircle2 = Circle((SNco[0], SNco[1]), aprad+skyrad, facecolor='none',
                    edgecolor='b', linewidth=2, alpha=1)

            #everything between skycircle1 and skycircle2 will be used to calculate background
